...
export default function PostCard({ post }) {
  ...
  return (
    <>
      <div className="w-full bg-white/10 backdrop-blur-xl shadow-2xl rounded-xl overflow-hidden cursor-pointer border border-purple-500/20 hover:shadow-purple-500/10 transition-all duration-300">
        {/* Header */}
            ...
            {descExpanded ? (
              <div>
                ...
                <button
                  ...
                  className="text-purple-400 hover:text-purple-300 cursor-pointer font-medium mt-2"
                >
                  Show less
                </button>
              </div>
          ...
          <div className="flex items-center space-x-6">
            <button
              ...
              className={`flex items-center space-x-2 transition-colors ${
                liked ? "text-red-500 hover:text-red-600" : "text-purple-300 hover:text-red-500"
              }`}
            >
          ...
          <div className="p-4 border-t border-purple-500/20 space-y-3">
            {comments.map((c) => (
              <div key={c._id || c.createdAt} className="flex items-start space-x-2">
                <img
                  ...
                  alt={c.userId?.name}
                  className="w-8 h-8 rounded-full object-cover flex-shrink-0 border border-purple-500/30"
                  onError={(e) => {
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2">
                    <span className="font-semibold text-white">{c.userId?.name || "User"}</span>
            ...
            <div className="flex mt-2 space-x-2">
              <input
                ...
                className="flex-1 px-3 py-2 border border-purple-500/30 rounded-lg focus:outline-none bg-white/10 backdrop-blur-sm text-white placeholder-purple-300 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              />
              <button
                onClick={handleAddComment}
                className="px-4 py-2 bg-gradient-to-r from-purple-600 to-purple-800 text-white rounded-lg hover:from-purple-700 hover:to-purple-900 transition-all shadow-lg hover:shadow-purple-500/25"
              >
                Post
              </button>
            </div>
