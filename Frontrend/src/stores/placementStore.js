...
export const usePlacementStore = create((set, get) => ({
  ...
  fetchYears: async () => {
    try {
      const res = await axios.get(`${API}/api/placements/years`, {
        withCredentials: true,
      });
  ...
  addPlacement: async (placementData) => {
    set({ isUpdating: true });
    try {
      const res = await axios.post(`${API}/api/placements`, placementData, {
        withCredentials: true,
      });
      ...
      if (res.data.success) {
        ...
        toast.success(
          res.data.message || "Placement achievement added successfully"
        );
        return true;
      } else {
        toast.error(
          res.data.message || "Failed to add placement achievement"
        );
        return false;
      }
    } catch (error) {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to add placement achievement"
      );
      return false;
    } finally {
  ...
  deletePlacement: async (placementId) => {
    ...
    try {
      const res = await axios.delete(
        `${API}/api/placements/${placementId}`,
        {
          withCredentials: true,
        }
      );
      ...
      if (res.data.success) {
        set({
          placements: get().placements.filter(
            (placement) => placement._id !== placementId
          ),
        });
        toast.success(
          res.data.message || "Placement achievement deleted"
        );
      }
    } catch (error) {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to delete placement achievement"
      );
    } finally {
      set({ isUpdating: false });
    }
