...
export const acceptConnectionRequest = async (req, res) => {
  try {
    ...
    // Check if current user already has this connection to avoid duplicates
    const existingConnection = currentUser.connections.find(
      conn => conn.user.toString() === userId
    );

    if (!existingConnection) {
      // Add to current user's connections only if it doesn't exist
      currentUser.connections.push({
        user: userId,
        status: 'connected',
...
// Clean up duplicate connections for a user
export const cleanupDuplicateConnections = async (req, res) => {
  try {
    ...
    for (const connection of currentUser.connections) {
      const userId = connection.user.toString();
      
      if (!connectionMap.has(userId)) {
        connectionMap.set(userId, connection);
        cleanedConnections.push(connection);
      } else {
        // If we already have a connection with this user, keep the one with 'connected' status
        const existing = connectionMap.get(userId);
        if (connection.status === 'connected' && existing.status !== 'connected') {
          // Replace with the connected one
          const index = cleanedConnections.findIndex(c => c.user.toString() === userId);
          cleanedConnections[index] = connection;
          connectionMap.set(userId, connection);
        }
      }
    }

    // Update user's connections with cleaned array
    currentUser.connections = cleanedConnections;
    await currentUser.save();

    return res.status(200).json({
      message: `Cleaned up connections. Removed ${currentUser.connections.length - cleanedConnections.length} duplicates.`,
      success: true,
      originalCount: currentUser.connections.length,
      cleanedCount: cleanedConnections.length
    });
  } catch (error) {
    console.error("Cleanup Duplicate Connections Error:", error);
    return res.status(500).json({ message: "Server error", success: false });
  }
};
